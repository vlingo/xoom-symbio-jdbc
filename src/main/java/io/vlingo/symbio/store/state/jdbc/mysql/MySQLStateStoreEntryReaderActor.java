...
public class MySQLStateStoreEntryReaderActor<T extends Entry<?>> extends Actor implements StateStoreEntryReader<T> {
	...
	public MySQLStateStoreEntryReaderActor(final EntryReader.Advice advice, final String name) throws Exception {
		this.advice = advice;
		this.name = name;
	...
	public Completes<Long> size() {
		try (final ResultSet resultSet = queryCount.executeQuery()) {
			...
			logger().error("vlingo/symbio-mysql: " + e.getMessage(), e);
			logger().error("vlingo/symbio-mysql: Rewinding the offset");
		}
	...
	private Entry<?> entryFrom(final ResultSet result, final long id) throws Exception {
		...
		if (configuration.format.isBinary()) {
			return new BaseEntry.BinaryEntry(String.valueOf(id), typed(type), typeVersion, binaryDataFrom(result, 4), metadata);
		} else {
			return new BaseEntry.TextEntry(String.valueOf(id), typed(type), typeVersion, textDataFrom(result, 4), metadata);
		}
	}
	...
	private long retrieveLatestOffset() {
		try {
			...
			logger().error("vlingo/symbio-mysql: Could not retrieve latest offset, using current.");
		}
	...
	private void updateCurrentOffset() {
		try {
			...
			logger().error("vlingo/symbio-mysql: Could not persist the offset. Will retry on next read.");
			logger().error("vlingo/symbio-mysql: " + e.getMessage(), e);
		}
	}
