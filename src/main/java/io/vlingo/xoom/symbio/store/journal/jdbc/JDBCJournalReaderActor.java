...
public class JDBCJournalReaderActor extends Actor implements JournalReader<TextEntry> {
  ...
  public Completes<TextEntry> readNext() {
    try (final Connection connection = connectionProvider.connection();
         final PreparedStatement selectEntry = queries.prepareSelectEntryQuery(connection, offset);
      ...
      logger().error("xoom-symbio-jdbc:journal-reader-" + databaseType + " readNext(): " + e.getMessage(), e);
    }
  ...
  public Completes<List<TextEntry>> readNext(final int maximumEntries) {
    try (final Connection connection = connectionProvider.connection();
         final PreparedStatement selectQueryBatch = queries.prepareSelectEntryBatchQuery(connection, offset, maximumEntries);
      ...
      logger().error("xoom-symbio-jdbc:journal-reader-" + databaseType + " readNext(int) : " + e.getMessage(), e);
    }
  ...
  public Completes<String> seekTo(final String id) {
    try (Connection connection = connectionProvider.connection()) {
      ...
      logger().error("xoom-symbio-jdbc:journal-reader-" + databaseType + " seekTo(String): " + e.getMessage(), e);
    }
  ...
  public Completes<Long> size() {
    try (final Connection connection = connectionProvider.connection();
         final PreparedStatement selectJournalCount = queries.prepareSelectJournalCount(connection);
      ...
      logger().error("xoom-symbio-jdbc:journal-reader-" + databaseType + " size(): " + e.getMessage(), e);
  ...
  private List<TextEntry> readIds(final List<Long> ids) {
    try (final Connection connection = connectionProvider.connection();
         final PreparedStatement statement = queries.prepareNewSelectEntriesByIdsQuery(connection, ids);
         final ResultSet resultSet = statement.executeQuery()) {
      final List<TextEntry> entries = entriesFromResultSet(resultSet);
      return entries;
    } catch (Exception e) {
      logger().error("xoom-symbio-jdbc:journal-reader-" + databaseType + " readIds(Connection, List<Long>): " + e.getMessage(), e);
