...
import io.vlingo.xoom.symbio.store.object.jdbc.jpa.model.converters.LocalDateTimeConverter;

/**
 * JPAEntry is an implementation of {@link io.vlingo.xoom.symbio.store.dispatch.Dispatchable} that is designed
 * to be persisted via the Java Persistence API
 */
@Entity(name = "Dispatchables")
@Table(name="tbl_xoom_objectstore_dispatchables")
@NamedNativeQueries({
        @NamedNativeQuery(
                name = "Dispatchables.deleteByDispatchId",
                query  = "DELETE FROM tbl_xoom_objectstore_dispatchables WHERE dispatch_id = ?"
        ),
})
@NamedQueries({
        @NamedQuery(
